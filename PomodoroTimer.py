...
def pomRound():
    ...
    totbreak_label.config(bg=mood, fg=status_color)
    totwork_label.config(bg=mood, fg=status_color)
    worktime_label.config(bg=mood, fg=status_color)
    breaktime_label.config(bg=mood, fg=status_color)
    t = 25 * 60
    while t > -1:
        mins = (t//60)
        secs = (t%60)
...
################################################_BUTTONS_###################################################
#start button
start_btn = Button(frame1, text = "Start", command = lambda:pomRound(), font = ("Big Caslon", 24), fg = "#226644", height = 2, width = 8)
